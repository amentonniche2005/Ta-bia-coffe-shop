...
socket.on('update_stock', () => { chargerStock(); });
socket.on('nouvelle_commande', (data) => { chargerCommandes(); });
socket.on('mise_a_jour_commande', () => { chargerCommandes(); });
...
async function chargerCatalogue() {
    try {
        ...
        document.getElementById("menuGrid").innerHTML = "<p class='empty-message' style='grid-column: 1/-1; text-align:center;'>❌ Impossible de charger le menu.</p>";
    }
}
...
function executerAjoutPanier(produit, variante) {
    ...
    if (produit.stock !== undefined) {
        ...
        if (quantiteTotalePanier >= produit.stock) return afficherNotification("Désolé, stock insuffisant !", "error");
    }
...
function changerQuantite(cartId, delta) {
    ...
    if (delta > 0 && produitDB && produitDB.stock !== undefined) {
        ...
        if (quantiteTotalePanier >= produitDB.stock) return afficherNotification("Stock insuffisant", "error");
    }
...
function afficherContenuPanier() {
    ...
    if (panier.length === 0) {
        ...
        totalElement.textContent = "Total: 0.00 DT"; document.getElementById("checkoutBtn").disabled = true; return;
    }
...
function passerCommande() { if (panier.length === 0) return; fermerPanier(); afficherModalTable(); }
...
function afficherModalTable() {
    ...
    modal.querySelectorAll('button[data-table]').forEach(btn => {
        btn.onclick = () => { const numTable = btn.getAttribute('data-table'); modal.remove(); afficherModalCode(numTable); };
    });
}
...
function afficherModalCode(numTable) {
    ...
    validerBtn.onclick = async () => {
        ...
        validerBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Vérification...'; validerBtn.disabled = true;
        let authValid = false, clientData = null;
        try {
        ...
        if (authValid) { modal.remove(); validerCommande(numTable, clientData, codeSaisi); } 
        else { afficherNotification("❌ Code incorrect ou expiré", "error"); codeInput.value = ""; validerBtn.innerHTML = 'Confirmer la commande'; validerBtn.disabled = false; }
    };
}
    ...
    try {
        ...
        const response = await fetch('/api/commandes', {
            ...
            body: JSON.stringify({
                ...
                numeroTable: tableFinale, clientId: idFidele, clientName: nomFidele, total: totalCommande,
                methodePaiementRequete: methode 
            })
        });
        ...
        } else { afficherNotification("❌ Erreur serveur lors de la commande", "error"); }
    } catch (e) { afficherNotification("❌ Erreur de connexion au serveur", "error"); 
    } finally {
...
function initClientSocket() {
    ...
    socket.on('mise_a_jour_commande', (commande) => {
        ...
        if(idx !== -1) {
            ...
            chargerMesCommandes();
            if(commande.statut === 'terminee') { afficherNotification("Votre commande est prête ! 🍽️"); if(navigator.vibrate) navigator.vibrate([100, 50, 100]); }
        }
    });
}
